...
Everyone Active gym bookings CLI + MCP server. **Read this file, [OPENCLAW.md](OPENCLAW.md), and [docs/agents.md](docs/agents.md) before using tools.**
...
4. **Activity queries:** use compact names (`hiit`, `combat`) — not spaced portal labels (`h I I t`).
5. **`list_members`**: multi-profile setups return **`data.profiles`** (not `members`). Single `.env` accounts still use **`data.members`**.
6. **`list_bookings`**: use each session's **`members`** array (no singular `member` field). Mention **`status`** when it is `Waiting List`.
7. **`book_class` `confirmed: false`** is not always failure — verify with `list_bookings` before retrying.
8. Set MCP **timeout ≥ 180s** — Playwright calls often take 30–90s. **Reload MCP** after editing `.eacli-profiles.json`.
9. On **`NETWORK_ERROR`**: wait, retry once, then force `login` + `doctor`. Read `.eacli-session/last-run.log` / `last-failure.html`.

## Documentation map

| Doc | Purpose |
|-----|---------|
| [OPENCLAW.md](OPENCLAW.md) | **OpenClaw first-stop** — MCP setup, skill, playbook, logs |
